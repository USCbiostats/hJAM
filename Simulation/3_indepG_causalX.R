...
numSNP = 20; numGenes = 2  # genotype set up
pi1 = pi2 = c(0,0.1)
pigroup = length(pi1)*length(pi2)
iterationTimes = 1000
SIGMA_sim = getSIGMA(numSNP/numGenes, numGenes, NULL, presetcorrG = 0)
...
for (g1 in 1:length(pi1)){
  for (g2 in 1:length(pi2)){
    ...
    for (k in 1:iterationTimes){
      ...
      Ex[,1] = rnorm(N_Gx, mean=Gx%*%A[,1],sd=1)
      corE = simBetabyR2(r2=0.2, Nrow=1, Ncol=1, X=Ex[,1], varY=1, sd=1)
      Ex[,2] = rnorm(N_Gx, mean=Gx%*%A[,2]+Ex[,1]*corE, sd=1)
      
      Ey[,1] = rnorm(N_Gy, mean=Gy%*%A[,1],sd=1)
      Ey[,2] = rnorm(N_Gy, mean=Gy%*%A[,2]+Ey[,1]*corE, sd=1)
...
write.table(out.cond.1, paste0("numSNP_",numSNP,"_indepG-causalX-simData1.txt"), quote=F, row.names = F, col.names = T)
