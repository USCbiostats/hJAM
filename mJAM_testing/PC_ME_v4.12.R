...
source('/project/dconti_624/Users/shenjiay/Anqi/scripts/mJAM_fn_in_progress/mJAM_build_CS_v2_med_yty_test2.R')
...
Forward_Dir <- paste0("/project/haiman_625/Users/wanganqi/Biobank/META2021/JAM/results_v3/Rerun/" , CHR_center, "_", POS_lower, "_", POS_upper, "/")
...
sumstats2 <- sumstats1 %>% 
  filter(!is.na(P.value), MarkerName_old != "1:16936052:C:T", MarkerName != "1:152535918:C:G",
         (is.na(Eur_17_study_Nov2021_1.txt.Freq1)|(Eur_17_study_Nov2021_1.txt.Freq1>0&Eur_17_study_Nov2021_1.txt.Freq1<1)), 
         (is.na(AAPC_10_study_Oct2021_1.txt.Freq1)|(AAPC_10_study_Oct2021_1.txt.Freq1>0&AAPC_10_study_Oct2021_1.txt.Freq1<1)), 
         (is.na(Asian_5_study_Oct2021_1.txt.Freq1)|(Asian_5_study_Oct2021_1.txt.Freq1>0&Asian_5_study_Oct2021_1.txt.Freq1<1)), 
         (is.na(Hispanic_5_study_Oct2021_1.txt.Freq1)|(Hispanic_5_study_Oct2021_1.txt.Freq1>0&Hispanic_5_study_Oct2021_1.txt.Freq1<1)))
...
#########################
## read known SNP files 
known_regions = read.table("/project/haiman_625/Users/wanganqi/Biobank/META2021/JAM/region.txt", header = T) %>%
  filter(CHR == CHR_center,POS>=POS_lower,POS<=POS_upper) %>% 
  rename(MarkerName_old = MarkerName) %>% 
  left_join(select(sumstats2, c(MarkerName, MarkerName_old)), by = "MarkerName_old") %>% 
  filter(!is.na(MarkerName))
known_snps = known_regions$MarkerName
#########################
...
for(i in 1:length(Input_Dosage)){
  if (nrow(missing_dosage_tbl) > 0 && i%in%missing_dosage_tbl$missing_ethnic_idx){
    ...
    temp_Dosage_cor <- WGCNA::cor(Input_Dosage[[i]][,-temp_missing_snp_idx])^2
    ...
    Dosage_cor[[i]] <- WGCNA::cor(Input_Dosage[[i]])^2
  }
...
while(iter_count >= 0){
  ...
  if(iter_count ==0){
    if(min(chr1_EUR_sub$P.value) > 1e-6) {break}
    curr_index_snp <- chr1_EUR_sub %>% arrange(P.value) %>% head(1) %>% pull(MarkerName)
    ...
    if(newFS_RES$condp_min > condp_cut) {break}
    ## output all condp
    condp_matrix <- signif(newFS_RES$condp_mx, digits = 3)
    colnames(condp_matrix) <- paste0(rep(c("testing_SNP",selected_ids),each = 3), c(".beta", ".se", ".p"))
    rownames(condp_matrix) <- subset_EUR
    write.table(condp_matrix, paste0(Forward_Dir,o_pre,"index", iter_count+1,"_all_cond_est.txt"), quote = F,row.names = T, col.names = T, sep = '\t')
    ## output mJAM marginal p and meta marginal p 
  ...
  curr_99CS <- curr_CS %>%
    mutate(CumSum_Porb = cumsum(SD_Post_CS_Prob),
           EmpiricalCut = min(CumSum_Porb[CumSum_Porb >= 0.99], na.rm = T),
           duplicated_one = duplicated(CumSum_Porb),
           CS_in = ((CumSum_Porb <= EmpiricalCut) & !duplicated_one))%>%
    select(-duplicated_one)
  all_99CS <- rbind(all_99CS, filter(curr_99CS, CS_in == T))
  message(paste(length(curr_CS_snp), "CS SNPs selected for", curr_index_snp))
  
  if(iter_count == 0){
    curr_CS %>%
      left_join(marginal_est, by = c("CS_SNP" = "SNP")) %>%
      filter(Post_Model_Prob_Ratio<=1) %>% 
      ggplot(aes(x = -log10(mJAM_p), y = Post_Model_Prob_Ratio))+
      geom_point()+
      labs(title = "BF-based P(Model)")
    ggsave(paste0(Forward_Dir,o_pre,"index",iter_count+1,"BF_g_CS_PModel_mJAMp.png"), width = 8, height = 5)
    curr_CS %>%
      left_join(marginal_est, by = c("CS_SNP" = "SNP")) %>%
      ggplot(aes(x = -log10(mJAM_p), y = R2))+
      geom_point()+
      labs(title = "BF-based P(Model)")
    ggsave(paste0(Forward_Dir,o_pre,"index",iter_count+1,"BF_g_CS_R2_mJAMp.png"), width = 8, height = 5)
  }else{
    curr_CS %>%
      left_join(condp_est, by = c("CS_SNP" = "SNP")) %>%
      filter(Post_Model_Prob_Ratio<=1) %>% 
      ggplot(aes(x = -log10(mJAM_p), y = Post_Model_Prob_Ratio))+
      geom_point()+
      labs(title = "BF-based P(Model)")
    ggsave(paste0(Forward_Dir,o_pre,"index",iter_count+1,"BF_g_CS_PModel_condmJAMp.png"), width = 8, height = 5)
    curr_CS %>%
      left_join(condp_est, by = c("CS_SNP" = "SNP")) %>%
      ggplot(aes(x = -log10(mJAM_p), y = R2))+
      geom_point()+
      labs(title = "BF-based P(Model)")
    ggsave(paste0(Forward_Dir,o_pre,"index",iter_count+1,"BF_g_CS_R2_condmJAMp.png"), width = 8, height = 5)
  }
  
  curr_CS %>%
    filter(Post_Model_Prob_Ratio<=1) %>% 
    ggplot(aes(x = R2, y = log(Post_Model_Prob_Ratio)))+
    geom_point()+
    labs(title = "BF-based P(Model)")
  ggsave(paste0(Forward_Dir,o_pre,"index",iter_count+1,"_BF_g_CS_logPModel_R2.png"), width = 8, height = 5)
  curr_CS %>%
    filter(Post_Model_Prob_Ratio<=1) %>% 
    ggplot(aes(x = R2, y = Post_Model_Prob_Ratio))+
    geom_point()+
    labs(title = "BF-based P(Model)")
  ggsave(paste0(Forward_Dir,o_pre,"index",iter_count+1,"_BF_g_CS_PModel_R2.png"), width = 8, height = 5)
...
if(!is.null(selected_ids)){
  ...
  ###
  next_condp <- data.frame(SNP = subset_EUR,
                           MarkerName_old = sumstats3$MarkerName_old[match(subset_EUR, sumstats3$MarkerName)],
                           condp = newFS_RES$condp) %>%
    filter(!is.na(condp)) %>%
    mutate(condp=format(condp,scientific = T))
  write.table(next_condp, paste0(Forward_Dir, o_pre,"index_plus_one_condp.txt"),quote = F, row.names = F, col.names = T, sep = '\t')
  ###
  output_CS <- all_CS %>%
  ...
  write.table(output_CS, paste0(Forward_Dir,o_pre, "CS.txt"),quote = F, row.names = F, col.names = T, sep = '\t')
  write.table(all_99CS, paste0(Forward_Dir,o_pre, "99CS.txt"),quote = F, row.names = F, col.names = T, sep = '\t')
  ###
  write.table(pruned_snp_df, paste0(Forward_Dir, o_pre,"pruned_SNPs.txt"),quote = F, row.names = F, col.names = T, sep = '\t')
  ###

  if(length(c(selected_ids, known_snps))>1){
    index_corr_df <- as_tibble(expand.grid.unique(c(selected_ids, known_snps), c(selected_ids,known_snps)))
    EURr2 <- AAr2 <- LAr2 <- Asianr2 <- rep(NA, nrow(index_corr_df))
    for(i in 1:nrow(index_corr_df)){
      temp_index1 <- as.character(index_corr_df[i,1])
      temp_index2 <- as.character(index_corr_df[i,2])
      EURr2[i] <- Dosage_cor[[1]][temp_index1, temp_index2]
      AAr2[i] <- Dosage_cor[[2]][temp_index1, temp_index2]
      LAr2[i] <- Dosage_cor[[3]][temp_index1, temp_index2]
      Asianr2[i] <- Dosage_cor[[4]][temp_index1, temp_index2]
    }
    index_corr_df$EURr2 = EURr2; index_corr_df$AAr2 = AAr2
    index_corr_df$LAr2 = LAr2; index_corr_df$Asianr2 = Asianr2
    index_corr_df = index_corr_df %>%
      left_join(select(sumstats3, c(MarkerName, MarkerName_old)), by = c("V1" = "MarkerName")) %>%
      rename(MarkerName_old_V1 = MarkerName_old) %>%
      left_join(select(sumstats3, c(MarkerName, MarkerName_old)), by = c("V2" = "MarkerName")) %>%
      rename(MarkerName_old_V2 = MarkerName_old) %>%
      mutate(EURr2 = as.numeric(EURr2), EURr2 = format(EURr2, digits = 3),
             AAr2 = as.numeric(AAr2), AAr2 = format(AAr2, digits = 3),
             LAr2 = as.numeric(LAr2), LAr2 = format(LAr2, digits = 3),
             Asianr2 = as.numeric(Asianr2), Asianr2 = format(Asianr2, digits = 3)) %>%
      select(c(MarkerName_old_V1, MarkerName_old_V2, EURr2, AAr2,LAr2,Asianr2, V1, V2))
    write.table(index_corr_df,
                paste0(Forward_Dir, o_pre,"index_corr.txt"),quote = F, row.names = F, col.names = T, sep = '\t')
  }
  ...
  ggsave(paste0(Forward_Dir,o_pre,"plot_cs_multi_ethnic.png"), width = 9, height = 5)
  ...
  ggsave(paste0(Forward_Dir,"plot_no_cs.png"), width = 8, height = 5)
}
