...
missing_id = as.numeric(sim_args[8:length(sim_args)])
missing_study = 1:3
#####
# sim_scenario = "J"
# n_study_per_pop = 3
...
Causal_Folder = paste0(N_SNP,"SNPs_",length(Causal_ID),"Causal_",paste(Causal_ID,collapse="_"),"_Cor", LD_corr, "_M", paste(missing_id,collapse="_"))
...
for (k in Start_Rep:(Start_Rep+N_Rep-1)){
  ...
  ## --- insert missingness
  Marg_Output[missing_id, c(2*missing_study, 2*missing_study+1)] <- NA
  MAF_Output[missing_id, missing_study+1] <- NA
